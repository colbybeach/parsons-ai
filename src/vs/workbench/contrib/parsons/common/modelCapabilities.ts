...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './parsonsSettingsTypes.js';
...
export type VoidStaticModelInfo = { // not stateful
	// Parsons uses the information below to know how to handle each model.
...
const extensiveModelOptionsFallback: VoidStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {
	...
	if (lower.includes('gemini') && lower.includes('3.1') && lower.includes('lite')) return toFallback(geminiModelOptions, 'gemini-3.1-flash-lite')
	...
	if (lower.includes('claude-fable-5')) return toFallback(anthropicModelOptions, 'claude-fable-5')
	if (lower.includes('claude-opus-4-8')) return toFallback(anthropicModelOptions, 'claude-opus-4-8')
	if (lower.includes('claude-sonnet-4-6')) return toFallback(anthropicModelOptions, 'claude-sonnet-4-6')
	if (lower.includes('claude-haiku-4-5')) return toFallback(anthropicModelOptions, 'claude-haiku-4-5')
	if (lower.includes('claude')) return toFallback(anthropicModelOptions, 'claude-3-7-sonnet-20250219')

	if (lower.includes('grok-4.3')) return toFallback(xAIModelOptions, 'grok-4.3')
	if (lower.includes('grok-build')) return toFallback(xAIModelOptions, 'grok-build-0.1')
	...
	if (lower.includes('gpt-5.4-mini')) return toFallback(openAIModelOptions, 'gpt-5.4-mini')
	if (lower.includes('gpt-5.4')) return toFallback(openAIModelOptions, 'gpt-5.4')
	if (lower.includes('gpt-5.3-codex')) return toFallback(openAIModelOptions, 'gpt-5.3-codex')
...
const anthropicModelOptions = {
	'claude-fable-5': {
		...
		// Adaptive thinking is always on and does not use the legacy `thinking` payload.
		reasoningCapabilities: false,
	},
	'claude-opus-4-8': {
		contextWindow: 1_000_000,
	...
	'claude-sonnet-4-6': {
		...
		reasoningCapabilities: {
			...
			reasoningSlider: { type: 'budget_slider', min: 1024, max: 16_384, default: 1024 },
		},
	},
	'claude-haiku-4-5': {
...
const anthropicSettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		if (lower.includes('claude-haiku-4-5')) fallbackName = 'claude-haiku-4-5'
...
const openAIModelOptions = { // https://platform.openai.com/docs/pricing
	...
	'gpt-5.4-mini': {
		...
		specialToolFormat: 'openai-style',
		supportsSystemMessage: 'developer-role',
		reasoningCapabilities: { supportsReasoning: true, canTurnOffReasoning: true, canIOReasoning: false, reasoningSlider: { type: 'effort_slider', values: ['none', 'low', 'medium', 'high'], default: 'none' } },
	},
	'gpt-5.3-codex': {
		contextWindow: 400_000,
		reservedOutputTokenSpace: 128_000,
...
const openAISettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		if (lower.includes('gpt-5.4-mini')) { fallbackName = 'gpt-5.4-mini' }
...
const xAIModelOptions = {
	...
	'grok-4.3': {
		contextWindow: 1_000_000,
		reservedOutputTokenSpace: 32_768,
		cost: { input: 1.25, output: 2.50, cache_read: 0.20 },
		downloadable: false,
		supportsFIM: false,
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
		reasoningCapabilities: { supportsReasoning: true, canTurnOffReasoning: true, canIOReasoning: false, reasoningSlider: { type: 'effort_slider', values: ['none', 'low', 'medium', 'high'], default: 'medium' } },
	},
	'grok-build-0.1': {
...
const xAISettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		if (lower.includes('grok-4.3')) fallbackName = 'grok-4.3'
		if (lower.includes('grok-build')) fallbackName = 'grok-build-0.1'
...
const geminiSettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		if (lower.includes('gemini-3.5-flash')) {
			return { modelName: 'gemini-3.5-flash', recognizedModelName: 'gemini-3.5-flash', ...geminiModelOptions['gemini-3.5-flash'] }
		}
		if (lower.includes('gemini-3.1-pro')) {
			return { modelName: 'gemini-3.1-pro-preview', recognizedModelName: 'gemini-3.1-pro-preview', ...geminiModelOptions['gemini-3.1-pro-preview'] }
		}
		if (lower.includes('gemini-3.1-flash-lite')) {
			return { modelName: 'gemini-3.1-flash-lite', recognizedModelName: 'gemini-3.1-flash-lite', ...geminiModelOptions['gemini-3.1-flash-lite'] }
		}
...
const groqModelOptions = { // https://console.groq.com/docs/models, https://groq.com/pricing/
	'openai/gpt-oss-120b': {
		...
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
		reasoningCapabilities: { supportsReasoning: true, canIOReasoning: true, canTurnOffReasoning: true, reasoningSlider: { type: 'effort_slider', values: ['low', 'medium', 'high'], default: 'medium' } },
	},
	'openai/gpt-oss-20b': {
		contextWindow: 131_072,
		reservedOutputTokenSpace: 65_536,

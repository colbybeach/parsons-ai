...
import { VoidCheckUpdateRespose } from './parsonsUpdateServiceTypes.js';
...
export class VoidUpdateService implements IVoidUpdateService {
	...
	readonly _serviceBrand: undefined;
	private readonly parsonsUpdateService: IVoidUpdateService;

	constructor(
		@IMainProcessService mainProcessService: IMainProcessService, // (only usable on client side)
	) {
		// creates an IPC proxy to use metricsMainService.ts
		this.parsonsUpdateService = ProxyChannel.toService<IVoidUpdateService>(mainProcessService.getChannel('void-channel-update'));
	}


	// anything transmitted over a channel must be async even if it looks like it doesn't have to be
	check: IVoidUpdateService['check'] = async (explicit) => {
		const res = await this.parsonsUpdateService.check(explicit)
		return res
	}
}

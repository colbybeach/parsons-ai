...
import { ModelDropdown, } from '../parsons-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { VOID_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../parsonsSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/parsons/common/parsonsSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../parsons-settings-tsx/WarningBox.js';
...
import { LearningActivity, LearningActivityResponse } from '../../../../common/chatQuizTypes.js';
import { acceptAllBg, acceptBorder, buttonFontSize, buttonTextColor, rejectAllBg, rejectBg, rejectBorder } from '../../../../common/helpers/colors.js';
import { builtinToolNames, isABuiltinToolName, MAX_FILE_CHARS_PAGE, MAX_TERMINAL_INACTIVE_TIME } from '../../../../common/prompt/prompts.js';
import { RawToolCallObj } from '../../../../common/sendLLMMessageTypes.js';
import ErrorBoundary from './ErrorBoundary.js';
import { ToolApprovalTypeSwitch } from '../parsons-settings-tsx/Settings.js';
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSwitch
				...
				onChange={(newVal) => {
					...
					parsonsSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					...
					parsonsSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					parsonsSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		parsonsSettingsService.setGlobalSetting('chatMode', newVal)
...
const LearningQuestionModal = ({ toolMessage, assistantMessage, assistantMessageIdx, threadId, onCloseForToolId }: {
	...
	threadId: string,
	onCloseForToolId: (toolId: string) => void
}) => {
	const accessor = useAccessor()
	const chatThreadsService = accessor.get('IChatThreadService')
...
export const SidebarChat = () => {
	...
	const [closedToolRequestIds, setClosedToolRequestIds] = useState<Set<string>>(new Set())
	...
	const visiblePendingToolRequest = useMemo(() => {
		if (!pendingToolRequest) return null
		if (closedToolRequestIds.has(pendingToolRequest.toolMessage.id)) return null
		return pendingToolRequest
	}, [closedToolRequestIds, pendingToolRequest])
	const onCloseForToolId = useCallback((toolId: string) => {
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-void-fg-3 select-none'>
		{[
			...
			'Create a .parsonsrules file for me'
		].map((text, index) => (
			<div
				key={index}
	...
	return (
		...
		>
			{isLandingPage ?
				landingPageContent
				: threadPageContent}
			{visiblePendingToolRequest ? <LearningQuestionModal
				toolMessage={visiblePendingToolRequest.toolMessage}
				assistantMessage={visiblePendingToolRequest.assistantMessage}
				assistantMessageIdx={visiblePendingToolRequest.assistantMessageIdx}
				threadId={threadId}
				onCloseForToolId={onCloseForToolId}
			/> : null}
		</Fragment>
	)
}

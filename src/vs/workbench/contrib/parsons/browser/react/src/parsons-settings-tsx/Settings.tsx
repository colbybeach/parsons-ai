...
const SimpleModelSettingsDialog = ({
	...
	const sourcecodeOverridesLink = `https://github.com/colbybeach/parsons-ai/blob/main/src/vs/workbench/contrib/parsons/common/modelCapabilities.ts#L146-L172`
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Parsons, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Parsons.`
							: `Parsons recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='parsons-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='parsons-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							<button
								...
								data-tooltip-id='parsons-tooltip'
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const parsonsSettingsService = accessor.get('IVoidSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		parsonsSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [parsonsSettingsService, providerName, settingName]);
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = parsonsSettingsService.state.settingsOfProvider[providerName].enabled
// 						parsonsSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [parsonsSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	return <ButtonLeftTextRightOption
		leftButton={<VoidSwitch
			...
			value={enabled}
			onChange={(newVal) => {
				parsonsSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <VoidInputBox2
		...
		onChangeText={(newText) => {
			parsonsSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		parsonsSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [parsonsSettingsService])

	return <VoidCustomDropdownBox
		className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={parsonsSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		<div
			...
			data-tooltip-id='parsons-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Parsons automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	return <div
		...
		onClick={() => { parsonsSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		parsonsSettingsService.setGlobalSetting('autoApprove', {
			...parsonsSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [parsonsSettingsService, metricsService])
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			dataStr = JSON.stringify(parsonsSettingsService.state, null, 2)
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					parsonsSettingsService.dangerousSetState(json as any)
				}
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Parsons' Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Parsons can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Parsons can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<VoidSwitch
																...
																onChange={(newVal) => parsonsSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => parsonsSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => parsonsSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => parsonsSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of Parsons suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => parsonsSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => parsonsSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into Parsons.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer Parsons' settings and chats in and out of Parsons.`}</h4>
									<div className='flex flex-col gap-8'>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { parsonsSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Parsons running smoothly. You may opt out below. Regardless of this setting, Parsons never sees your code, messages, or API keys.</h4>
...
Alternatively, place a \`.parsonsrules\` file in the root of your workspace.
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<VoidSwitch
													...
													onChange={(newValue) => {
														parsonsSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, Parsons will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>

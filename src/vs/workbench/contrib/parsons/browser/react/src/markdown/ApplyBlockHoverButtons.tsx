...
export const useApplyStreamState = ({ applyBoxId }: { applyBoxId: string }) => {
	...
	const getStreamState = useCallback(() => {
		...
		return parsonsCommandBarService.getStreamState(uri)
	}, [parsonsCommandBarService, applyBoxId])
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: { tooltipName: string, color?: IndicatorColor, position?: PlacesType, offset?: number }) => ({
	'data-tooltip-id': color === 'orange' ? `parsons-tooltip-orange` : color === 'green' ? 'parsons-tooltip-green' : 'parsons-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	...
	useCommandBarURIListener(useCallback((uri_) => {
		...
		if (shouldUpdate) { setStreamState(parsonsCommandBarService.getStreamState(uri)) }
	}, [parsonsCommandBarService, applyBoxId, uri]))
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`Parsons Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`Parsons Error: There was a problem running Apply: ${e}.`)

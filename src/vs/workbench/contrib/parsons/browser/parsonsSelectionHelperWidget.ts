...
import { mountParsonsSelectionHelper } from './react/out/parsons-editor-widgets-tsx/index.js';
...
export type ParsonsSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	...
	constructor(
		...
		@IVoidSettingsService private readonly _parsonsSettingsService: IVoidSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountParsonsSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._parsonsSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies ParsonsSelectionHelperProps)

...
import { FeatureName } from '../common/parsonsSettingsTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IVoidModelService private readonly _parsonsModelService: IVoidModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._parsonsModelService.initializeModel(model.uri)
	...
	// 		message: `Parsons Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'void.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Parsons's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._parsonsModelService.getModel(uri)
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('Parsons 1') }
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._parsonsModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Parsons: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._parsonsModelService.initializeModel(uri)
		await this._parsonsModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Parsons: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Parsons: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Parsons error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IParsonsCommandBarService private readonly _parsonsCommandBarService: IParsonsCommandBarService,
		...
		const getAcceptRejectText = () => {
			...
			const commandBarStateAtUri = this._parsonsCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._parsonsCommandBarService.onDidChangeState(e => {

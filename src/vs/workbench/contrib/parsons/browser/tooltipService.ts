...
import { mountParsonsTooltip } from './react/out/parsons-tooltip/index.js';
...
export class TooltipContribution extends Disposable implements IWorkbenchContribution {
	...
	private initializeTooltip(): void {
		...
		if (workbench) {
			...
			const tooltipContainer = h('div.parsons-tooltip-container').root;
			workbench.appendChild(tooltipContainer);

			// Mount the React component
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountParsonsTooltip(tooltipContainer, accessor);

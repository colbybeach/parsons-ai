...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		@IVoidModelService parsonsModelService: IVoidModelService,
		@IEditCodeService editCodeService: IEditCodeService,
		@ITerminalToolService private readonly terminalToolService: ITerminalToolService,
		@IParsonsCommandBarService private readonly commandBarService: IParsonsCommandBarService,
		...
		this.validateParams = {
			...
			edit_file: (params: RawToolParamsObj) => {
				...
				if (extractSearchReplaceBlocks(searchReplaceBlocks).length === 0) {
					throw new Error('Invalid LLM output: edit_file.search_replace_blocks must contain at least one valid Search/Replace block.')
				}
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				await parsonsModelService.initializeModel(uri)
				const { model } = await parsonsModelService.getModelSafe(uri)
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = parsonsModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.parsonsSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by Parsons after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}

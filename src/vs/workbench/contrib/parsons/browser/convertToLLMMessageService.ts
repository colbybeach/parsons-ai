...
import { IVoidSettingsService } from '../common/parsonsSettingsService.js';
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/parsonsSettingsTypes.js';
...
export const combineChatSystemMessage = (baseSystemMessage: string, extraSystemMessage: string | undefined, disableSystemMessage: boolean): string => {
	if (disableSystemMessage) return extraSystemMessage?.trim() ?? ''
	return [baseSystemMessage, extraSystemMessage?.trim()].filter(Boolean).join('\n\n')
}
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .parsonsrules file):\n${aiInstructions}`)
...
export interface IConvertToLLMMessageService {
	...
	prepareLLMChatMessages: (opts: { chatMessages: ChatMessage[], chatMode: ChatMode, modelSelection: ModelSelection | null, extraSystemMessage?: string }) => Promise<{ messages: LLMChatMessage[], separateSystemMessage: string | undefined }>
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IVoidSettingsService private readonly parsonsSettingsService: IVoidSettingsService,
		@IVoidModelService private readonly parsonsModelService: IVoidModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .parsonsrules files from workspace folders
	private _getVoidRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let voidRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.parsonsrules')
				const { model } = this.parsonsModelService.getModel(uri)
				if (!model) continue
	...
	// Get combined AI instructions from settings and .parsonsrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.parsonsSettingsService.state.globalSettings.aiInstructions;
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.parsonsSettingsService.state
		...
		const modelSelectionOptions = this.parsonsSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection, extraSystemMessage }) => {
		if (modelSelection === null) return { messages: [], separateSystemMessage: undefined }
		...
		const { disableSystemMessage } = this.parsonsSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = combineChatSystemMessage(fullSystemMessage, extraSystemMessage, disableSystemMessage)

		const modelSelectionOptions = this.parsonsSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]

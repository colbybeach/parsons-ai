...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Parsons
	...
	async initialize() {
		...
		const { commit, version, parsonsVersion, release, quality } = this._productService
		...
		console.log('User is opted out of basic Parsons metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Parsons posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
